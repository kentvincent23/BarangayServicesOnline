...
<body class="bg-[#f8fafc] min-h-screen pb-12">
    ...
    <main class="max-w-6xl mx-auto mt-6 md:mt-10 px-4 md:px-6">
            ...
            <div class="flex flex-col md:flex-row md:items-center md:justify-between md:gap-16 py-6 md:py-7 md:min-h-[70vh]">
                ...
                <div class="flex-1 flex flex-col items-center text-center md:items-end md:text-right space-y-5 md:space-y-6 order-1 md:order-2 mb-8 md:mb-0">
                    ...
                    <div class="flex justify-center md:justify-end gap-3 md:gap-4 w-full">
                        <a href="{{ route('login') }}" class="flex-1 md:flex-none text-center bg-white border border-slate-200 text-slate-700 px-6 py-3 md:px-8 md:py-4 rounded-2xl font-black uppercase tracking-widest text-xs transition-all hover:bg-blue-700 hover:text-white shadow-sm">
                            Sign In
                        </a>
                        <a href="{{ route('register') }}" class="flex-1 md:flex-none text-center bg-blue-700 text-white px-6 py-3 md:px-8 md:py-4 rounded-2xl font-black uppercase tracking-widest text-xs transition-all hover:bg-blue-800 shadow-xl shadow-blue-100">
                            Create Account
                        </a>
                    </div>
                </div>
            ...
            <header class="flex flex-col md:flex-row md:items-end justify-between gap-6">
                ...
                <div class="flex gap-3 flex-wrap">
                    ...
                    <div class="bg-white p-5 rounded-[2rem] shadow-sm border border-slate-200 flex items-center gap-4">
                        ...
                        <div>
                            <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Ready to Pickup</p>
        ...
        <tr class="bg-slate-50 border-b border-slate-100">
            <th class="w-1/4 px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest">Resident / ID</th>
            <th class="w-1/4 px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest">Requested Document</th>
            <th class="w-1/6 px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">Verification</th>
            <th class="w-1/6 px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">Status</th>
            <th class="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest text-right">Actions</th>
                ...
                @if($app->id_image_path)
                    {{-- This button is styled to be prominent but fits the modern UI --}}
                    <a href="{{ asset('storage/' . $app->id_image_path) }}" target="_blank" 
                       class="inline-flex items-center gap-1.5 px-3 py-2 bg-amber-50 text-amber-600 rounded-xl hover:bg-amber-600 hover:text-white transition-all text-[10px] font-black uppercase tracking-widest shadow-inner shadow-amber-100/50 border border-amber-200/50">
                ...
                @else
                    {{-- Shows when image path is NULL or Empty --}}
                    <span class="inline-flex items-center gap-1.5 px-3 py-2 text-[10px] font-black uppercase tracking-widest text-slate-300 italic">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
<div class="grid grid-cols-1 sm:grid-cols-5 gap-3 sm:gap-4">
    ...
    {{-- M.I. (Now has its own dedicated 1/5th space) --}}
...
<div class="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
    ...
    <div class="space-y-2">
        ...
        <input type="text" name="civil_status" value="{{ old('civil_status', Auth::user()->civil_status ?? '') }}"
            class="w-full px-4 sm:px-5 py-3 sm:py-4 bg-slate-50 border-none rounded-xl sm:rounded-2xl outline-none focus:ring-2 focus:ring-blue-500 font-semibold text-sm" required>
    </div>
</div>
...
<div class="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
    <div class="space-y-2">
        ...
        <select name="service_type" class="w-full px-4 sm:px-5 py-3 sm:py-4 bg-slate-50 border-none rounded-xl sm:rounded-2xl outline-none focus:ring-2 focus:ring-blue-500 font-bold text-slate-700 text-sm appearance-none">
...
<div class="space-y-2 p-4 bg-blue-50/50 rounded-2xl border-2 border-dashed border-blue-100">
        ...
        <p class="text-[10px] text-slate-400 ml-1 italic font-medium">
            *This image is used only for verification and will be deleted once processed.
        </p>
    </div>
...
{{-- The filter logic stays the same --}}
<div x-show="filter === 'all' || filter === '{{ $app->status }}'" 
    ...
    <div class="min-w-0">
        {{-- Displaying the Document Type from your column name --}}
        <p class="font-black text-slate-900 text-sm sm:text-base leading-tight">
            {{ $app->document_type }}
        </p>
        
        {{-- The "scholar" or purpose text --}}
        <p class="text-[11px] text-slate-500 italic mt-0.5 truncate">
            "{{ $app->purpose }}"
        </p>
...
@endforeach
        ...
        {{-- No results message --}}
        <div x-show="filter !== 'all' && !document.querySelectorAll('[x-show*=\'' + filter + '\']').length" class="text-center py-10">
            <p class="text-slate-400 text-xs font-bold uppercase tracking-widest">No applications found for this status</p>
        </div>
    </div>
</div>
@endif
